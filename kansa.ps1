...
xml. The default output format is json. Kansa's user may specify 
        ...
        $Modules.Keys | Foreach-Object { $Module = $_
            ...
            if ($Pushbin) {
                ...
                if ($bindep) {
                    ...
                    # Send-file only supports one destination at a time. Therefore, if we detect more than 
                    # one remote directory, we loop through them. This is dependent on the number of remote
                    # directories and active remote sessions being the same. 
                    if ($RemoteWindir.Count -gt 1){
                        for ($i=0; $i -lt $PSSessions.Count; $i++){
                            $null = Send-File -Path (ls $bindep).FullName -Destination $RemoteWindir[$i] -Session $PSSessions[$i]
                        }
                    }
                    else{
            ...
            # Check for a directive specifying a custom output format
            $customOutput = ($DirectivesHash.Get_Item("OUTPUT"))

            # If an output directive is specified, set the output format accordingly
            if ($customOutput){
                $OutputFormat = $DirectivesHash.Get_Item("OUTPUT")
            }
            # If no output directive is specified, revert to original output format
            # supplied when kansa was launched
            else{
            ...
            $Job.ChildJobs | Foreach-Object { $ChildJob = $_
                ...
                switch -Wildcard ($OutputFormat) {
                    ...
                    "*hybrid" {
					    # This option was built to accomodate exporting multiple file types for each module
					    $OutfileC = $Outfile + ".csv"
                        $Recpt | Export-Csv -NoTypeInformation -Encoding $Encoding $OutfileC
					
					    $OutfileJ = $Outfile + ".json"
                        $Recpt | ConvertTo-Json -Depth $JSONDepth -Compress | Set-Content -Encoding $Encoding $OutfileJ
				    }
...
$SubmitChangeMgtEventScriptblock_Str = @'
##############################
#.SYNOPSIS
#Notify Change Management REST endpoint of a Kansa event
#
#.DESCRIPTION
#Submit an event to Change Management REST endpoint
#   
#
##############################
Param (
...
# Store the initial format specified at invocation.
...
# Get our targets. #
if ($TargetList) {
    [System.Collections.ArrayList]$Targets = Get-Targets -TargetList $TargetList
} elseif ($Target) {
